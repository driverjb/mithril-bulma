...
export const classSchema = z
  ...
  .default('');
export const responsive = z.boolean().default(false);
// .transform((r) => (r ? 'is-responsive' : ''));
export const size = z.enum(['small', 'normal', 'medium', 'large']).default('normal');
// .transform((s) => (s === 'normal' ? '' : `is-${s}`));
export const fullWidth = z.boolean().default(false);
// .transform((f) => (f ? 'is-fullwidth' : ''));
export const rounded = z.boolean().default(false);
// .transform((r) => (r ? 'is-rounded' : ''));
/** force the "hovered" style of the element without it being hovered */
export const hovered = z.boolean().default(false);
// .transform((f) => (f ? 'is-hovered' : ''));
/** force the "focused" style of the element without it being focused */
export const focused = z.boolean().default(false);
// .transform((f) => (f ? 'is-focused' : ''));
/** force the "active" style of the element without it being active */
export const active = z.boolean().default(false);
// .transform((f) => (f ? 'is-active' : ''));
/** display a loading spinner */
export const loading = z.boolean().default(false);
// .transform((f) => (f ? 'is-loading' : ''));
export const isStatic = z.boolean().default(false);
